...
export function GlobeCanvas({ className = '' }: { className?: string }) {
  ...
  const { land, ocean, cityVectors } = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      // great-circle links between cities
      ctx.lineWidth = 1.1
      for (const [a, b] of LINKS) {
        ...
        ctx.strokeStyle = 'rgba(0, 217, 154, 0.42)'
        ctx.stroke()
      }

      // city markers with a pulsing halo
      const pulse = (Math.sin(now / 700) + 1) / 2
      for (const city of cityVectors) {
        const p = project(city.v, cos, sin, cosT, sinT, cx, cy, r)
        if (p.z <= 0.05) continue
        const base = city.primary ? 3.6 : 2.4
        ctx.globalAlpha = 0.25 + p.z * 0.5
        ctx.beginPath()
        ctx.arc(p.x, p.y, base + pulse * (city.primary ? 9 : 5), 0, Math.PI * 2)
        ctx.strokeStyle = city.primary ? 'rgba(0, 217, 154, 0.55)' : 'rgba(124, 243, 206, 0.35)'
        ctx.lineWidth = 1
        ctx.stroke()
        ...
        ctx.fillStyle = city.primary ? '#00d99a' : '#7cf3ce'
        ctx.shadowBlur = 14
      ...
      ctx.beginPath()
      ctx.arc(cx, cy, r, 0, Math.PI * 2)
      ctx.strokeStyle = 'rgba(124, 243, 206, 0.28)'
      ctx.lineWidth = 1.2
      ctx.stroke()
